...
IcmpMessage	new_icmp_echo_message(ProgramConf *conf) {
	...
	message.icmp.icmp_cksum = calculate_icmp_checksum(&message, sizeof(IcmpMessage));
	return (message);
}
...
TracerouteMessage new_message(ProgramConf *conf, TracerouteMessagePayloadType message_type) {
	...
	new_message.iphdr = (struct iphdr) {
...
void set_iphdr(ProgramConf *conf, struct iphdr *iphdr) {
